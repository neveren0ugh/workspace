...
int main() {
    ...
    double h = 10. / (point_number - 1), knots[point_number], knots1[point_number];
    knots[0] = 0;
    for (int i = 0; i < point_number; i++) {
        knots[i] = (x_right + x_left) / 2. + (x_right - x_left) * cos((2 * i + 1) * M_PI / (2 * point_number)) / 2;
    }
    const char csv_file_name[64] = "work4.csv";
    std::ofstream csv_file;
    csv_file.open(csv_file_name);
    for (size_t i = 0; i < point_number; ++i){
        csv_file << (knots[i] + knots[i + 1]) / 2 << "," << polynom(knots, point_number, (knots[i] + knots[i + 1]) / 2, knots1) << "\n";
    }
    csv_file.close();
    for ( int i = 0; i < point_number - 1; i ++) {
        cout << (knots[i] + knots[i + 1]) / 2 << "  "  << polynom(knots, point_number, (knots[i] + knots[i + 1]) / 2, knots1) << "\n";
    }
